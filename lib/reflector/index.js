    ...
    async.whilst(
      ...
      function(whilstdone) {
        ...
        if (!ip) {
          _.defer(function() {
            // TOP ERROR!
            cb(new Error('top', 'Network error(No available servers) for ' +
              'service(%s), method(%s).', that.service.url, method));
            whilstdone(1);
          });
        } else {
          that.service.connector.connect(ip, method, params, function(err, result) {
            if (err && err.type === 'network') {
              ...
              if (retries < 0) {
                cb(new Error('top', 'Network error(%s) for service(%s), ' +
                  'method(%s), params(%j).', err.info, that.service.url, params));
                whilstdone(1);
              }
            ...
            if (err) {
              cb(new Error('top', 'Server error(%s) for service(%s), method(%s), ' +
                'params(%j)', err.info, that.service.url, method, params));
            }
